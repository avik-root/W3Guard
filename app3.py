...
Admin password is generated securely on first run
"""
...
def admin_dashboard():
    ...
    # Load all data
    users = user_handler.load_data()  # This was missing!
    scans = scan_handler.load_data()
    ...
    return render_template('admin/dashboard.html',
                         users=users,  # This was missing!
                         user_emails=user_emails,  # Add this for easy lookup
...
if __name__ == '__main__':
    ...
    users = user_handler.load_data()
    admin_exists = any(user.get('is_admin') for user in users.values())
    
    if not admin_exists:
        admin_id = str(uuid.uuid4())
        
        # Get admin email from environment or use default
        admin_email = os.getenv('ADMIN_EMAIL', 'admin@w3guard.edu')
        
        # Get initial password from environment or generate secure one
        admin_password_plain = os.getenv('ADMIN_INITIAL_PASSWORD', None)
        if admin_password_plain is None:
            admin_password_plain = generate_secure_password()
            # Save temporary credentials to file (for reference only)
            credentials_file = 'data/admin_credentials.txt'
            with open(credentials_file, 'w') as f:
                f.write(f"ADMIN CREDENTIALS (Temporary - Delete after first login)\n")
                f.write(f"{'='*50}\n")
                f.write(f"Email: {admin_email}\n")
                f.write(f"Password: {admin_password_plain}\n")
                f.write(f"{'='*50}\n")
                f.write(f"IMPORTANT: Delete this file after changing the password!\n")
            print(f"\n{'='*60}")
            print(f"ADMIN ACCOUNT CREATED")
            print(f"Email: {admin_email}")
            print(f"Temporary credentials saved to: data/admin_credentials.txt")
            print(f"{'='*60}")
            print(f"IMPORTANT: Delete admin_credentials.txt after first login!")
            print(f"{'='*60}\n")
        
        admin_password_hashed = bcrypt.generate_password_hash(admin_password_plain).decode('utf-8')
        users[admin_id] = {
            'id': admin_id,
            'email': admin_email,
